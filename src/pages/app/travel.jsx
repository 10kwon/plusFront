...
export const TravelPage = (props) => {
  ...
  useEffect(() => {
    axios.get('https://mapi.pcor.me/api/board/list.php?id=58&page=1')
...
<Breadcrumb class="pt-6 md:px-4" aria-label="Default breadcrumb example">
            ...
            <Breadcrumb.Item>
              {boardData.board && boardData.board.name} {/* 여기서 제목을 표시합니다 */}
            </Breadcrumb.Item>
          </Breadcrumb>
<h3
...
<p class="flex items-center pt-4 pb-12 md:px-4 font-semibold text-lg
          capitalize dark:text-gray-300">
            {boardData.board && boardData.board.description}</p>
...
<Card href={`https://mapi.pcor.me/view.php?id=${post.postid}`} className="mb-2" key={post.postid}>
